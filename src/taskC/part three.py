...
#find nearest object and face it; need to specify lowest angle to highest angle for the range.
...
# currently need to calibrate it to go back into range when it finishes turning. ALSO turning isn't always successful. 
def turn_around(): 
 ...
 while True: 
  min_value,min_pos = find_nearest(-90,120) #RMB pos can range from -90 to 150
  print min_pos
  print min_value
  #print value
  #print pos
  ...
  if min_pos in range (-100,100): #if the object is in front of the robot still 
   if correction > 0: # too far from the desired dist
    ...
    motor2.run_timed(duty_cycle_sp = 0.01*correction+30, time_sp = 400) # we turn right towards the object
   ...
   if correction < 0: # too close to the desired dist 
    ...
    motor1.run_timed(duty_cycle_sp = -0.01*correction+30, time_sp = 400) # we turn left away from the object
  ...
  if ((min_pos in range(100,150)) | (min_pos in range(-120,-20))): # when the object is likely to be give bad readings 
   ...
   while min_value > 300:
    ...
    motor1.polarity = 'normal' 
    motor2.polarity = 'normal' 
    motor1.run_timed(duty_cycle_sp = 0.01*n_correction+30, time_sp = 100)
    motor2.run_timed(duty_cycle_sp = 0.01*n_correction+30, time_sp = 100)
    time.sleep(1) 
    motor1.run_timed(duty_cycle_sp = 0.01*n_correction+30, time_sp = 600)
...
#keep going straight until the object id within 10cm then stop. medium motor stays still. 
def run_till_found():  
...
def turn_around(): 
 ...
 while True:
  ...
  if correction > 0: #too far from desired dist 
   motor2.polarity = 'normal'
   motor1.polarity = 'normal'
   motor2.run_timed(duty_cycle_sp = 0.01*correction+30, time_sp = 300) # we turn right towards the object
   
  if correction < 0: # too close to the desired dist 
   motor1.polarity = 'inversed'
   motor2.polarity = 'inversed'
   motor1.run_timed(duty_cycle_sp = -0.01*correction+30, time_sp = 300) # we turn left away from the object
  if correction == 0:  
  # motor1.polarity = 'normal'
  # motor2.polarity = 'normal'
   motor1.run_timed(duty_cycle_sp = 40, time_sp = 300)
